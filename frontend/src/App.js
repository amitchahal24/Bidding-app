...
function App() {
    ...
    useEffect(() => {
        fetch("https://bidding-app-6c8i.onrender.com/api/products")
            .then((res) => res.json())
    ...
    return ( <
...

        {
            products.length > 0 && ( <
                ...
                onChange = {
                    (e) => handleInputChange(e, products[currentIndex]._id) }
                /> <
                input type = "number"
                name = "amount"
                ...
                /> <
                button onClick = {
                    () => handleBidSubmit(products[currentIndex]._id) } >
                Submit Bid <
                /button>

                <
                button onClick = {
                    () => handleClearBids(products[currentIndex]._id) }
                style = {
                    { backgroundColor: "red", marginTop: "5px", color: "white", padding: "7px", border: "none", borderRadius: "5px", cursor: "pointer" } } >
                Clear Bid History <
                /button>
